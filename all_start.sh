...
nohup python dn_bot.py > dn_bot.log 2>&1 &
...
sleep 1
cd ~/binance/kraken
nohup python kraken_bot.py > kraken_bot.log 2>&1 &

echo "=== KRAKEN XSTOCK WATCH ==="
pkill -f xstock_watch.py 2>/dev/null || true
sleep 1
cd ~/binance/kraken
nohup python xstock_watch.py > xstock_watch.log 2>&1 &

echo "=== IPO NVDA ==="
pkill -f "ipo.py --profile nvda" 2>/dev/null || true
sleep 1
cd ~/binance/121trade
nohup python ipo.py --profile nvda > nvda.log 2>&1 &

echo "=== IPO SPCX ==="
pkill -f "ipo.py --profile spcx" 2>/dev/null || true
sleep 1
cd ~/binance/121trade
nohup python ipo.py --profile spcx > spcx.log 2>&1 &
