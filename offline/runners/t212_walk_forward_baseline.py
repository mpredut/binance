...
def main() -> int:
    ...
    parser.add_argument("--fx-to-usd", type=float, help="obligatoriu dacă STRAT_CURRENCY != USD")
    ...
    fx_to_usd = args.fx_to_usd
    if params.currency == "USD":
        fx_to_usd = 1.0 if fx_to_usd is None else fx_to_usd
    elif fx_to_usd is None:
        parser.error("--fx-to-usd este obligatoriu pentru un profil non-USD")
    if fx_to_usd is None or fx_to_usd <= 0:
    ...
    result = evaluate_walk_forward(
        records,
        lambda ohlc, warmup_ohlc: t212_replay.run_replay(
            ohlc,
            params,
