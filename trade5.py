...
class TrendState:
    ...
    def is_trend_fresh(self, fresh_trend_time=self.fresh_trend_time):
...
trend_state1 = TrendState(max_duration_seconds= 2.5 * 60 * 60, expiration_trend_time=10 * 60, fresh_trend_time = 1.7 * 60)  # Expira In 10 minute
trend_state2 = TrendState(max_duration_seconds= 2.5 * 60 * 60, expiration_trend_time=10 * 60, fresh_trend_time = 1.7 * 60)  # Expira In 10 minute
...
while True:
    #try:
        ...
        slope_big, _ = price_window_big.check_price_change(PRICE_CHANGE_THRESHOLD__BIG_EUR)
        if gradient > 0 and slope != 0 :
            # Confirmam un trend de crestere
            print(f"DIFERENTA MARE BIG UP!")
            proposed_price = current_price # * (1 - 0.01)
            if trend_state2.is_trend_up():
                count = trend_state2.confirm_trend() # Confirmam ca trendul de crestere continua         
                #25 de confirmari per minut * 1.5 minute
                if trend_state2.is_trend_up() < 25 * 1.5 and trend_state2.is_trend_fresh(): 
                    #track_and_place_order('BUY', sym.btcsymbol, count, proposed_price, current_price, order_ids=order_ids)
                    api.place_order_smart("BUY", self.symbol, proposed_price, 0.017, 
        ...
        if gradient < 0 and slope != 0 :
            # Confirmam un trend de scadere
            print(f"DIFERENTA MARE BIG DOWN!")
            proposed_price = current_price #  * (1 + 0.01)
            if trend_state2.is_trend_down():
                count = trend_state2.confirm_trend() # Confirmam ca trendul de scadere continua
                if trend_state2.is_trend_down() < 25 * 1.5 and trend_state2.is_trend_fresh() :
                    #track_and_place_order('SELL', sym.btcsymbol, count, proposed_price, current_price, order_ids=order_ids)
                    api.place_order_smart("SELL", self.symbol, proposed_price, 0.017, 
                        force=True, cancelorders=True, hours=1)
            else:
                expired_trend = trend_state2.start_trend('DOWN')  # Incepem un trend nou de scadere
                #track_and_place_order('SELL', sym.btcsymbol, 1, proposed_price, current_price, order_ids=order_ids)
        ...
        if gradient <= 0 and (trend_state2.is_trend_up() > 25 * 3 or trend_state2.is_trend_old(TREND_TO_BE_OLD_SECONDS)) :
            api.place_order_smart("SELL", self.symbol, proposed_price, 0.2, 
                force=True, cancelorders=True, hours=1)
        #25 de confirmari per minut * 3 minute
        if gradient >= 0 and (trend_state2.is_trend_down() > 25 * 3 or trend_state2.is_trend_old(TREND_TO_BE_OLD_SECONDS)) :
            api.place_order_smart("BUY", self.symbol, proposed_price, 0.2, 
                force=True, cancelorders=True, hours=1)
