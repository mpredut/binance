...
RETRY_INTERVAL_SEC de la ultima incercare) il REIA prin mkt.place() la PRET CURENT
(recalculat, in avantaj), cu is_retry=True (ca sa NU se re-enqueue-ze). Succes -> scos din
coada. Esec -> attempts++/last_attempt, ramane. Peste TTL (sau plafon incercari) -> scos +
ALERTA (renuntare, interventie manuala).
...
def main():
    ...
    if not oq.RETRY_ENABLED:
        print("[order_retry] RETRY_ENABLED=false — worker inactiv, ies.")
        return 0
