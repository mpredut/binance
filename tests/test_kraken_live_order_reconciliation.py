...
try:
    _SPEC = importlib.util.spec_from_file_location(
        "kraken_live_reconciliation_under_test",
        os.path.join(KRAKEN_DIR, "strategy.py"),
    )
...
class KrakenLiveReconciliationTest(unittest.TestCase):
    def test_open_partial_fill_applies_only_new_cumulative_delta(self):
        ...
        engine.client.query_orders.return_value = {
            "BUY-1": {
                "status": "open", "vol_exec": "0.4", "cost": "40",
                "fee": "0.10",
            }
        }
        ...
        engine.client.query_orders.return_value["BUY-1"].update(
            vol_exec="0.7", cost="71", fee="0.18",
        )
        with patch.object(strat, "notify"):
    ...
    def test_canceled_partial_buy_is_accounted_then_removed_once(self):
        ...
        engine.client.query_orders.return_value = {
            "BUY-1": {
                "status": "canceled", "vol_exec": "0.25", "cost": "24.5",
                "fee": "0.07",
            }
        }
    ...
    def test_incremental_sell_closes_cycle_only_when_order_is_terminal(self):
        ...
        engine.client.query_orders.return_value = {
            "SELL-1": {
                "status": "open", "vol_exec": "0.4", "cost": "44",
                "fee": "0.10",
            }
        }
        ...
        engine.client.query_orders.return_value["SELL-1"].update(
            status="closed", vol_exec="1.0", cost="112", fee="0.28",
        )
        with patch.object(strat, "notify"):
    ...
    def test_cancel_failure_keeps_order_tracked(self):
        ...
        state["orders"].append(_buy_order())
        engine = _strategy(state=state)
        engine.client.cancel_order.side_effect = strat.KrakenError("timeout")
    ...
    def test_stop_does_not_create_conflicting_exit_when_cancel_fails(self):
        ...
        engine.client.add_order.assert_not_called()
        self.assertIsNotNone(engine._find_open("buy"))
    ...
    def test_failed_stop_order_is_not_announced_as_executing(self):
        ...
        engine = _strategy(state=state, stop_loss_pct=10.0)
        engine.client.add_order.side_effect = strat.KrakenError("insufficient funds")
