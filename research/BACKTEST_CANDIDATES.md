...
| 4 | `instruments.conf` `[BINANCE_BTC]` | `mt.gain` / `mt.lost` | 6.0% / 3.55% (28 iul, aplicat de pilot) | 🟡 **APLICAT, dar de RE-VALIDAT** (29 iul) | Confirmat pe 2 ferestre (gain: winner 5.0, marja +33.78/+112.78; lost: winner 3.8, marja +331.42/+324.08) — medie cu valoarea veche (7.0/3.3): 6.0/3.55. Propus prin pipeline dev->git->apply_proposals.py, aplicat manual de user. **DAR**: verdictul a rulat cu `is_trend_up()` NEUTRALIZAT (mereu False, via `scheduled_pilot.py::_run_one` / `_neutral_is_trend_up`) — testeaza de fapt "vinde mereu la prag, niciodata nu astepta continuarea trendului". Cu semnal REAL (ReplayTrendSource, 29 iul), o rulare de test pe `run_replay_backtest.py` a dat rezultate SEMNIFICATIV diferite pt BTC (buys 39->121, net -272.83->-855.07) — comportamentul se schimba real cu semnalul, nu doar zgomot. `scheduled_pilot.py` (motorul care a produs verdictul APLICAT) ramane INCA pe varianta neutralizata (neatins, decizie user) — verdictul de mai sus n-a fost re-testat cu semnalul real. |
| 5 | `instruments.conf` `[BINANCE_TAO]` | `mt.gain` / `mt.lost` | 9.2% / 5.25% (aplicat 23-24 iul) | 🟡 **APLICAT, dar de RE-VALIDAT** (29 iul) | `mt.lost` 4.9->5.25 (pilot, confirmat pe 2 ferestre, singurul din 4 chei TAO). `mt.gain` ramane 9.2 (fara semnal confirmat). Acelasi caveat ca #4: verdictul a rulat cu `is_trend_up()` neutralizat, nu re-testat inca cu ReplayTrendSource (semnal real). |
...
| 8 | `tradeall_config.env` | `TRADEALL_FIRE_MIN_RETRY_MINUTES` | 6 min | 🟡 INCONCLUDENT (28 iul) | Sweep {3,4.5,6,9,12}min rulat pe felie densa 0.5z (BTC+TAO) — 0 BUY/0 SELL in TOATE configuratiile: fereastra prea scurta, NICIUN trend-start (conditia gradient/slope_big) nu s-a produs deloc. Parametrul conteaza DOAR cand exista un trend activ de refirit — netestabil fara o fereastra care sa contina macar un eveniment real (ca Exp 1, care a folosit 12h cu 1 eveniment cunoscut). De reluat cu fereastra tintita, nu una arbitrara. |
| 9 | `tradeall_config.env` | `TRADEALL_FIRE_MAX_PER_TREND` | 3 | 🟡 INCONCLUDENT (28 iul) | Acelasi sweep/aceeasi cauza ca #8 — 0 trend-starts in felia testata, deci 0 executii indiferent de plafon. Vezi nota #8. |
